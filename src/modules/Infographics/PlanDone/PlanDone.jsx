...
function PlanDone({data}) {
    ...
    let [chartLabel, setChartLabel] = React.useState([]); // Змінна chartLabel містить масив з найменуваннями секторів діаграми, який спочатку пустий
        ...
        <div className='plan-done'>
            ...
            <div className="plan-done__body">
                <div className="plan-done__doughnut">
                    <div className='plan-done__done'>{data[0]['percentDoneToYearCorrectionPlan'] ? data[0]['percentDoneToYearCorrectionPlan'] : '?'} %</div>
                    ...
                    <Doughnut id='plan-done-id'
                        ...
                        options={{
                            responsive: true,
                            aspectRatio: 2, //вдвічі ширша за висоту. Оскільки ширина у нас задана, то висота буде вдвічі менша
                            plugins: {
                                legend: {
                                    display: false,
                                }
                        ...
                        <ul className="plan-done-legend__list">
                            ...
                            <li className="plan-done-legend__item">
                                <span className="plan-done-legend__color" style={{backgroundColor: chartRef.current.data.datasets[0].backgroundColor[0]}}>
                                    {data[0]['percentDoneToYearCorrectionPlan'] ? data[0]['percentDoneToYearCorrectionPlan'].toFixed(2) : '?'} %
                                </span> 
                            ...
                            <li className="plan-done-legend__item">
                                <span className="plan-done-legend__color" style={{backgroundColor: chartRef.current.data.datasets[0].backgroundColor[1]}}>
                                    {data[0]['percentDoneToYearCorrectionPlan'] ? (100 - data[0]['percentDoneToYearCorrectionPlan']).toFixed(2) : "?"} % 
                                </span> 
